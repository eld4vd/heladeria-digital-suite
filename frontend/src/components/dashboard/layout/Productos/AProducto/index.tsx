...
import type { CreateProductoDto } from '../../../../../models/Producto';
...
const AgregarProducto = ({ abierto, onCerrar }: Props) => {
  const qc = useQueryClient();
  const [nombre, setNombre] = useState('');
  const [descripcion, setDescripcion] = useState('');
  const [sabor, setSabor] = useState('');
  const [precio, setPrecio] = useState(''); // string para mantener compatibilidad
  const [stock, setStock] = useState<number>(0);
  const [categoriaId, setCategoriaId] = useState<number | ''>('');
  const [activo, setActivo] = useState(true);
  // Campo de entrada y URL confirmada para previsualización
  const [imagenUrlInput, setImagenUrlInput] = useState('');
  const [imagenPreviewUrl, setImagenPreviewUrl] = useState('');
  const [previewError, setPreviewError] = useState<string | null>(null);

  // sin slug: trabajamos con URL directamente
  ...
  // sin efecto de archivo local

  const createMutation = useMutation({
    mutationFn: productService.createProducto,
    onSuccess: () => {
    ...
    onError: (error) => {
      toast.error(`Error al crear el producto: ${(error as Error).message}`, {
  ...
  // imagenUrl es opcional; si la proporcionas, debería parecer una URL
  // puedes reforzar validación si lo necesitas
    return '';
  };
        ...
        <div className="p-6 overflow-y-auto flex-1">
          ...
          <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="md:col-span-2">
              ...
              <p className="mt-1 text-xs text-gray-500">Usa una URL pública (ej. CDN/Cloudinary/Imgur directo con extensión). Campo opcional.</p>
              {previewError && (
